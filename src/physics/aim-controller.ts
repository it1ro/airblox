...
/** Дефолтные параметры spring-damper по правилам: kp ∈ [2..8], kd ∈ [0.5..2]. */
export const AIM_CONTROLLER_DEFAULTS = {
  /** Жёсткость по рысканью (yaw). */
  kpYaw: 4,
  /** Демпфирование по рысканью (yaw). */
  kdYaw: 1,
  /** Жёсткость по тангажу (pitch). */
  kpPitch: 4,
  /** Демпфирование по тангажу (pitch). */
  kdPitch: 1,
  /** Жёсткость по крену (roll, для auto-bank). */
  kpRoll: 4,
  /** Демпфирование по крену (roll, для auto-bank). */
  kdRoll: 1,
  /** Deadzone по углу ошибки (рад), ~0.5–1°. */
  deadzoneRad: (1 * Math.PI) / 180
} as const;
